...
module.exports = {
	entry: start,
	settings: {
		name: "Import Curiosa Collection",
		options: {},
	},
};
...
async function start(params) {
	...
	if (!csvFiles.length) {
		new Notice("No CSV files found in vault. Place your Curiosa export CSV in the vault first.", 6000);
		return;
	}
	...
	const csvFile = await QuickAdd.quickAddApi.suggester(
		...
		csvFiles,
		"Choose Curiosa export CSV",
	);
	if (!csvFile) return;

	const boxName = await QuickAdd.quickAddApi.inputPrompt(
		"Name for storage box:",
		"Curiosa Import",
		"Curiosa Import",
	);
	if (!boxName?.trim()) return;
	const trimmedBox = boxName.trim();

	await S.ensureStorageBox(config, trimmedBox);

	const content = await app.vault.adapter.read(csvFile.path);
	const { rows } = S.parseStorageCsv(content);
	if (!rows.length) {
	...
	if (notFound.length) {
		...
		const report = `# Import not found — ${trimmedBox}\n\n` +
